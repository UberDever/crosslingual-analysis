...
Пусть R это множество ссылок в графе, для всех r из R: r<sub>i</sub>.name == r<sub>j</sub>.name;

D это множество объявлений одного идентификатора в графе, для всех d из D: d<sub>i</sub>.name == d<sub>j</sub>.name;

Возможные ограничения C:
1. Для всех r из R и d из D: для каждой пары (r, d) d должен существовать. Иначе ошибка: Неразрешенная ссылка
1. Для всех d из D: каждый d обязан иметь как минимум одну ссылку. Иначе ошибка: Неиспользованное определение
1. Для всех d из D: каждый d обязан иметь не более одной ссылки. Иначе ошибка: Множество ссылок на неразделяемый ресурс
...
Заметим, что с данными ограниченями допускается существование множества одинаковых d. Это может вести
к can't resolve reference to declaration, поэтому имеет смысл ограничить возможные объемы множества D.
...
- [ ] Дополнить онтологию конструкциями, позволяющими задавать подграфы определенной структуры для
